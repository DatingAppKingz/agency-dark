#!/usr/bin/env python3
"""
Fix migration 037 - reorder table creation to avoid foreign key issues.
"""
from pathlib import Path

def fix_migration_037_order():
    """Fix migration 037 to create tables in the correct order."""
...
def upgrade() -> None:
    ...
    # Create notification_templates table FIRST (before notifications)
    ...
    if notifications_exists:
        ...
        if 'template_id' not in existing_columns:
            ...
            op.create_foreign_key('fk_notifications_template_id', 'notifications', 'notification_templates', ['template_id'], ['id'])
...
def downgrade() -> None:
    ...
    # If notifications table exists and has our columns, drop the foreign key first
    if 'template_id' in columns:
        ...
        for row in result:
            try:
                op.drop_constraint(row[0], 'notifications', type_='foreignkey')
            except:
                pass
    
    # Drop notification_templates after dropping foreign key
    result = conn.execute(sa.text(
        "SELECT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'notification_templates')"
    ))
    if result.scalar():
    ...
    # If notifications table has columns we added, remove them
    columns_to_drop = [
    ...
    if columns:  # Only if we have columns
    ...
    print("Fixed migration 037 table creation order")

if __name__ == '__main__':
    fix_migration_037_order()