...
def upgrade() -> None:
    ...
    # Check and create webhookstatus enum
    connection = op.get_bind()
    result = connection.execute(sa.text("SELECT 1 FROM pg_type WHERE typname = 'webhookstatus'"))
    if not result.fetchone():
        connection.execute(sa.text("CREATE TYPE webhookstatus AS ENUM ('active', 'inactive', 'failed')"))
    ...
    result = conn.execute(sa.text(
        "SELECT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'webhook_deliveries')"
    ))
    if not result.scalar():
    ...
    # Create index for webhook_logs (check if they exist)
    # These indexes might already exist from migration 021
    conn = op.get_bind()
    
    # Check and create each index
    result = conn.execute(sa.text(
        "SELECT 1 FROM pg_indexes WHERE indexname = 'idx_webhook_logs_provider_created'"
    ))
    if not result.fetchone():
        op.create_index('idx_webhook_logs_provider_created', 'webhook_logs', ['provider', 'created_at'])
    
    result = conn.execute(sa.text(
        "SELECT 1 FROM pg_indexes WHERE indexname = 'idx_webhook_logs_event_type'"
    ))
    if not result.fetchone():
        op.create_index('idx_webhook_logs_event_type', 'webhook_logs', ['event_type'])
    
    result = conn.execute(sa.text(
        "SELECT 1 FROM pg_indexes WHERE indexname = 'idx_webhook_logs_status'"
    ))
    if not result.fetchone():
