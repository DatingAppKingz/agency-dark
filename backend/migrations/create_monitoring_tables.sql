...
CREATE TYPE IF NOT EXISTS health_status AS ENUM (
    'healthy',
    'degraded',
    'unhealthy',
    'unknown'
...
CREATE TABLE IF NOT EXISTS alert_rules (
    ...
    query TEXT, -- Optional custom query
    aggregation VARCHAR(50), -- 'avg', 'sum', 'min', 'max', 'count'
...
-- Webhook deliveries table (for webhook monitoring)
...
INSERT INTO alert_rules (name, description, metric_type, condition, threshold, severity) VALUES
    ('High API Error Rate', 'Alert when API error rate exceeds 5%', 'api_error_rate', 'gt', 5, 'error'),
    ('Low Cache Hit Rate', 'Alert when cache hit rate drops below 80%', 'cache_hit_rate', 'lt', 80, 'warning'),
    ('High Database Response Time', 'Alert when database queries take over 1 second', 'database_query_time', 'gt', 1000, 'warning'),
    ('High Memory Usage', 'Alert when memory usage exceeds 90%', 'system_memory', 'gt', 90, 'critical'),
    ('High CPU Usage', 'Alert when CPU usage exceeds 80%', 'system_cpu', 'gt', 80, 'warning')
